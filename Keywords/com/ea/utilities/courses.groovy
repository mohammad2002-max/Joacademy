...
public class courses extends methods{
	...
	def CardCourseDetais(List<WebElement> elements) {
		...
		if(!driver.findElements(By.xpath(".//button[contains(text(), 'شراء الدورة المختارة')]")).isEmpty()) {
			
			println("the course inclide in the package")
			WebElement buyCard = randomElement.findElement(By.xpath(".//button[contains(text(), 'شراء الدورة المختارة')]"))
			this.hoverToTheButton(buyCard)
		}
